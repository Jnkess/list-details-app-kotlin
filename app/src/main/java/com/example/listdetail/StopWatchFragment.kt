...
class StopWatchFragment : Fragment(R.layout.fragment_stop_watch) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        saveButton.setOnClickListener {
            ...
            val record = sharedTime.getFloat("$type time", POSITIVE_INFINITY)
            val sdf = SimpleDateFormat("dd/M/yyyy")
            val currentDate = sdf.format(Date())

            val edit = sharedTime.edit()
            if (record > time.toFloat()) {
                edit.putString(type, timeTv.text.toString() + " at " + currentDate)
                edit.putFloat("$type time", time.toFloat())
                edit.apply()
                Toast.makeText(view.context, "You completed $type. Congrats!", Toast.LENGTH_LONG).show()
            }
            edit.putString("$type last", timeTv.text.toString() + " at " + currentDate)
            edit.apply()
            Toast.makeText(view.context, "You completed $type. Congrats!", Toast.LENGTH_LONG).show()
        }
