...
class MainActivity : AppCompatActivity(R.layout.activity_main), GestureDetector.OnGestureListener {
    private val soupDishes = ListFragment.newInstance("soup")
    private val mainDishes = ListFragment.newInstance("dish")
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        bottomNavigation.setOnNavigationItemSelectedListener {
            when(it.itemId) {
                ...
                R.id.ic_easy -> {
                    replaceFragment(soupDishes)
                    tabId = 1
                }
                R.id.ic_difficult -> {
                    replaceFragment(mainDishes)
                    tabId = 2
                }
            }
            true
